// Auto-generated winmm.dll proxy. Forwards every named winmm export to
// C:\Windows\System32\winmm.dll, and in DllMain LoadLibraries dayz_takaro.dll
// from the same folder. DayZServer_x64.exe imports from winmm.dll, so
// dropping this DLL named winmm.dll in the server dir hijacks the import.
...
#pragma comment(linker, "/export:[NONAME]=C:\\Windows\\System32\\winmm.[NONAME],@2")

static HMODULE g_takaroDll = nullptr;
...
BOOL APIENTRY DllMain(HMODULE hModule, DWORD reason, LPVOID lpReserved) {
    ...
    case DLL_PROCESS_ATTACH: {
        ...
        if (lastSlash) {
            *(lastSlash + 1) = '\0';
            char takaroPath[MAX_PATH];
            sprintf_s(takaroPath, "%sdayz_takaro.dll", selfPath);
            g_takaroDll = LoadLibraryA(takaroPath);
        }
        break;
    }
    case DLL_PROCESS_DETACH:
        if (g_takaroDll) {
            FreeLibrary(g_takaroDll);
            g_takaroDll = nullptr;
        }
        break;
    }
